...
若产品需要「哪个目录占空间」，必须单独设计：

| 机制 | 说明 |
|------|------|
| 显式触发 | 用户点击「分析」才跑，或按 cron（如每天 1 次） |
| 范围限制 | 只扫指定路径；禁止从 `/` 无差别扫 |
| 限深 / 采样 | `max_depth`、每层 Top-N、跳过 snaphots |
| 配额与熔断 | 墙钟超时（如 30s）、文件数上限、IO nice / `ionice` |
| 结果缓存 | 结果带 `as_of` 时间戳，前端展示「非实时」 |
| 与快路径隔离 | 独立 worker / 独立调度，绝不阻塞常规 heartbeat |
