...
| 目录限额告警 | **软配额**：`path_pattern`（如 `/home/*/*`）+ 定向测体积 + 超限 Alert | 慢路径 B |
| 真正写满拒绝 | **硬配额**：XFS project / 用户配额 / 独立挂载；Agent 只读账本展示 | OS，非热路径 walk |
...
| Soft vs Hard | 产品默认做软告警；硬限制靠 OS 配额，不宣称 Agent 拦截 `write` |
...
### Phase 3 — 深度存储分析与配额（谨慎开启）

- **3a** 异步目录 Top-N 下钻 + 报告缓存 + 大文件 Top-N（定位「谁占用」）  
- **3b** QuotaPolicy（如 `/home/*/*` 每文件夹限额）+ 定向扫描 + 超限告警  
- **3c**（可选）对接 XFS project / 用户配额账本只读展示  
...
6. 大目录定位与目录软/硬配额方案有独立文档，且不污染热路径  
...
8. 目录超限：仅软告警，还是同期上 OS 硬配额？告警是否通知目录 owner？  
